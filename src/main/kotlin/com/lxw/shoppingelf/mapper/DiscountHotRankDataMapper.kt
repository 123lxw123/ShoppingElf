...
interface DiscountHotRankDataMapper {
    ...
    @Update("UPDATE discount_hot_rank_data SET discount_hot_rank_date=#{discountHotRankDate}, uid=#{uid}, url=#{url}, through_url=#{throughUrl}, rank=#{rank}, title=#{title} ,price=#{price}, description=#{description}, source=#{source}, image=#{image}, date=#{date}, category=#{category}, introducer=#{introducer}, label=#{label}, share_count=#{shareCount}, collect_count=#{collectCount}, like_count=#{likeCount} WHERE url=#{url} AND discount_hot_rank_date = #{discountHotRankDate}")

...
interface DiscountHotRankDataMapper {
    ...
    @Select("SELECT * FROM discount_hot_rank_data WHERE id IN (SELECT MAX(id) FROM discount_hot_rank_data GROUP BY url) AND id < #{id} AND rank = '' ORDER BY id DESC LIMIT #{limit}")
